...
Tensor4D<int64_t> ternary_gemm(Tensor7D<int64_t>& activation, Tensor5D<int64_t>& weights) {
  ...
  const size_t kernel_number = weights.dim1;

  // We essentially reinterpret the tensors as 2D tensors and do
  // Matrix-Matrix multiplication.
  const size_t M = batch_size * output_height * output_width;
  // KH * KW * C * BITS
  const size_t K = activation.dim4 * activation.dim5 * activation.dim6 * activation.dim7;
  const size_t N = kernel_number;
  // sanity check: K (from activation) == KH * KW * C * BITS (from weights)
  assert(K == weights.dim2 * weights.dim3 * weights.dim4 * weights.dim5);

  // NOTE In the original code he initializes this to 0. Why?
  Tensor4D<int64_t> output (batch_size, output_height, output_width, kernel_number, false);
  ...
  for (size_t im = 0; im < M; im++) {
	...
	int64_t p1 = activation.get_123_4567(im, ik + 0) ^ weights.get_1_2345(in, ik + 0);
	int64_t p2 = activation.get_123_4567(im, ik + 1) & weights.get_1_2345(in, ik + 1);
	cntp1 += popcnt64(p2);
