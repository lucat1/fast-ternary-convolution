...
void conv(registry::conv_type_t type, int *btn_cnt1, float *input,
  ...
  output_height =
      (PackedH - kernel_height + 1) / stride_height;          // Output Height
  output_width = (PackedW - kernel_width + 1) / stride_width; // Output Width
  PackedC = (num_channels % CNTBITS)
                ? ((num_channels / CNTBITS) + 1)
                : (num_channels / CNTBITS); // The channel after bit-packing
  ...
  if ((type == registry::conv_type_t::TNN) ||
    ...
    quant_input = ternarize_NCHW_to_NHWCB(
        input, padding_height, padding_width, quant_threshold, batch_size,
        num_channels, input_height, input_width);
    quant_input = img2row_NHWCB_to_N_OHOW_KHKWC(
        quant_input.data(), batch_size, PackedC * BITS, PackedH, PackedW,
    ...
    quant_input =
        binarize_NCHW_to_NHWC(input, padding_height, padding_width, batch_size,
                              num_channels, input_height, input_width);
    quant_input = img2row_NHWCB_to_N_OHOW_KHKWC(
        quant_input.data(), batch_size, PackedC, PackedH, PackedW,
        kernel_height, kernel_width, stride_height, stride_width);
  }
  ...
  case registry::conv_type_t::TNN: {
    y_intermediate = tnn_gemm_baseline(
        quant_input.data(), quant_weights,
        batch_size * output_height * output_width, kernel_number,
        PackedC * kernel_height * kernel_width);
    break;
  }
  case registry::conv_type_t::TBN: {
    y_intermediate = tbn_gemm_baseline(
        quant_input.data(), quant_weights,
        batch_size * output_height * output_width, kernel_number,
        PackedC * kernel_height * kernel_width);
    break;
  ...
  case registry::conv_type_t::BTN: {
    y_intermediate = btn_gemm_baseline(
        quant_input.data(), quant_weights, btn_cnt1,
        batch_size * output_height * output_width, kernel_number,
        PackedC * kernel_height * kernel_width);
    break;
  }
  case registry::conv_type_t::BNN: {
    y_intermediate =
        bnn_gemm_baseline(quant_input.data(), quant_weights,
                          batch_size * output_height * output_width,
                          kernel_number, PackedC * kernel_height * kernel_width,
  ...
  y = PReLU(y_intermediate.data(), batch_size, kernel_number, output_height,
            output_width, relu_alpha);
  memcpy(output, y.data(),
         batch_size * num_channels * output_width * output_height);
  output = y.data();
}
