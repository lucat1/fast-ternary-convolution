#include "impl/tern2row_memcpy/tern2row_memcpy.hpp"
#include "common.hpp"
...
#define copy_all_channels(quantized_reshaped_data, out_h, out_w, kernel_h,     \
  ...
  {                                                                            \
    const size_t fbcpp = full_blocks_c + (channels % 64 ? 1 : 0);              \
    const size_t bytes = 2 * sizeof(int64_t) * fbcpp;                          \
    int64_t *const src = tensor7d_addr(                                        \
        quantized_reshaped_data, out_h, out_w, kernel_h, kernel_w, packed_c,   \
        2, in, io_h - offst_io_h, io_w - offst_io_w, ik_h + offst_ik_h,        \
        ik_w + offst_ik_w, 0, 0);                                              \
    int64_t *const dest = tensor7d_addr(quantized_reshaped_data, out_h, out_w, \
                                        kernel_h, kernel_w, packed_c, 2, in,   \
                                        io_h, io_w, ik_h, ik_w, 0, 0);         \
...
Tensor7D<int64_t> tern2row_memcpy(const Tensor4D<float> &data,
                                  const Tensor1D<float> &thresholds,
                                  const size_t padding_h,
                                  const size_t padding_w, const size_t kernel_h,
