#include "impl/merge_gemm_prelu_blocked/gemm.hpp"
#include "common.hpp"
#include <cassert>
#include <cstddef>
#include <cstdint>
...
namespace merge_gemm_prelu_blocked {
...
Tensor4D<float> ternary_gemm(const Tensor7D<int64_t> &activation,
      ...
      for (size_t im = im_blk; im < im_blk + block_size; im++) {
        for (size_t in = in_blk; in < in_blk + block_size; in++) {
          ...
          for (; ik + 5 * BITS < K; ik += 6 * BITS) {
            p11 = activation.get_123_4567(im, ik + 0) ^
                  kernel.get_1_2345(in, ik + 0);
            p21 = activation.get_123_4567(im, ik + 1) &
  ...
  for (; im_blk < M; im_blk++) {
    for (size_t in = 0; in < N; in++) {
      ...
      int64_t p21, p22, p23, p24, p25, p26;
      for (; ik + 5 * BITS < K; ik += 6 * BITS) {
        p11 = activation.get_123_4567(im_blk, ik + 0) ^
              kernel.get_1_2345(in, ik + 0);
        p21 = activation.get_123_4567(im_blk, ik + 1) &
              kernel.get_1_2345(in, ik + 1);
        cntp11 += popcnt64(p21);
        cntp21 += popcnt64(p11 & p21);
        p12 = activation.get_123_4567(im_blk, ik + BITS + 0) ^
              kernel.get_1_2345(in, ik + BITS + 0);
        p22 = activation.get_123_4567(im_blk, ik + BITS + 1) &
              kernel.get_1_2345(in, ik + BITS + 1);
        cntp12 += popcnt64(p22);
        cntp22 += popcnt64(p12 & p22);
        p13 = activation.get_123_4567(im_blk, ik + 2 * BITS + 0) ^
              kernel.get_1_2345(in, ik + 2 * BITS + 0);
        p23 = activation.get_123_4567(im_blk, ik + 2 * BITS + 1) &
              kernel.get_1_2345(in, ik + 2 * BITS + 1);
        cntp13 += popcnt64(p23);
        cntp23 += popcnt64(p13 & p23);
        p14 = activation.get_123_4567(im_blk, ik + 3 * BITS + 0) ^
              kernel.get_1_2345(in, ik + 3 * BITS + 0);
        p24 = activation.get_123_4567(im_blk, ik + 3 * BITS + 1) &
              kernel.get_1_2345(in, ik + 3 * BITS + 1);
        cntp14 += popcnt64(p24);
        cntp24 += popcnt64(p14 & p24);
        p15 = activation.get_123_4567(im_blk, ik + 4 * BITS + 0) ^
              kernel.get_1_2345(in, ik + 4 * BITS + 0);
        p25 = activation.get_123_4567(im_blk, ik + 4 * BITS + 1) &
              kernel.get_1_2345(in, ik + 4 * BITS + 1);
        cntp15 += popcnt64(p25);
        cntp25 += popcnt64(p15 & p25);
        p16 = activation.get_123_4567(im_blk, ik + 5 * BITS + 0) ^
              kernel.get_1_2345(in, ik + 5 * BITS + 0);
        p26 = activation.get_123_4567(im_blk, ik + 5 * BITS + 1) &
              kernel.get_1_2345(in, ik + 5 * BITS + 1);
        cntp16 += popcnt64(p26);
        cntp26 += popcnt64(p16 & p26);
      }
      ...
      int cntp2 = cntp21 + cntp22 + cntp23 + cntp24 + cntp25 + cntp26;
      for (; ik < K; ik += BITS) {
        int64_t p1 = activation.get_123_4567(im_blk, ik + 0) ^
                     kernel.get_1_2345(in, ik + 0);
        int64_t p2 = activation.get_123_4567(im_blk, ik + 1) &
                     kernel.get_1_2345(in, ik + 1);
        cntp1 += popcnt64(p2);
        cntp2 += popcnt64(p1 & p2);
      }
      const float current = cntp1 - cntp2 - cntp2;
      if (current > 0) {
        output.set_123_4(current, im_blk, in);
      } else {
        output.set_123_4(current * alpha, im_blk, in);
      }
    }
  }
