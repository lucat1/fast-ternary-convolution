...
#include "impl/nhwc_tmacro1/gemm.hpp"
#include "impl/nhwc_tmacro1/prelu.hpp"
#include "impl/tern2row/tern2row.hpp"
#include "measure.hpp"
...
Tensor4D<float> conv(const Tensor4D<float> &input,
  ...
  Tensor7D<int64_t> quantized_reshaped =
      tern2row(input, thresholds, padding_h, padding_w, kernel.dim2,
  ...
  auto gemm_result = nhwc_tmacro1::ternary_gemm(quantized_reshaped, kernel);
  measure_point(measurement_point::gemm, MeasurementEvent::END);

  // activation
  measure_point(measurement_point::prelu, MeasurementEvent::START);
  auto result = nhwc_tmacro1::prelu(gemm_result, relu_alpha);
