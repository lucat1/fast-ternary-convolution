...
#include "main_impls/data_order_nhwc/gemm.hpp"
#include "main_impls/data_order_nhwc/im2row.hpp"
#include "main_impls/data_order_nhwc/prelu.hpp"
#include "measure.hpp"
...
Tensor4D<float> conv(const Tensor4D<float> &input,
  ...
  Tensor7D<int64_t> reshaped = data_order_nhwc::im2row(
  ...
  auto gemm_result = data_order_nhwc::ternary_gemm(reshaped, kernel);
  measure_point(measurement_point::gemm, MeasurementEvent::END);

  // activation
  measure_point(measurement_point::prelu, MeasurementEvent::START);
  auto result = data_order_nhwc::prelu(gemm_result, relu_alpha);
