...
VerificationReport *verify() {
  ...
  for (auto tc : test_cases) {
    for (auto conv_type : convolution_types) {
      ...
      std::vector<float> px =
          DirectPad(data->x, data->padding_size.height,
      ...
      if ((data->padding_size.width > 0 || data->padding_size.height > 0) &&
        ...
        cmp = Compare_Tensor_BNN_Padding(
            ...
            data->y_shape.height, data->y_shape.width,
      ...
      if (cmp > 0)
        report->passed.push_back(data);
      else
        report->failed.push_back(data);
    }
  }
  return report;
}
